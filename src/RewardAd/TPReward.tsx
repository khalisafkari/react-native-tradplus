...
const Module = NativeModules.TPReward;
...
const setRewardAdExListener = (callback: {
  onAdAgainImpression: (info: TPAdInfo) => void;
  onAdAgainVideoStart: (info: TPAdInfo) => void;
  onAdAgainVideoEnd: (info: TPAdInfo) => void;
  onAdAgainVideoClicked: (info: TPAdInfo) => void;
  onAdPlayAgainReward: (info: TPAdInfo) => void;
}) => {
  Module.setRewardAdExListener();
  const EventName: EmitterSubscription[] = [];
  for (let i of Object.keys(callback)) {
    const cb = callback as any;
    EventName.push(NativeEvent.addListener(i, cb[i]));
...
const RewardInit = (unitId: string) => {
  Module.RewardInit(unitId);
};
